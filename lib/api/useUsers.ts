...
export function ChangeRole() {
  ...
  return useMutation({
    mutationFn: async (vals: { userId: string, role: Role }): Promise<boolean> => {
      if (vals.role === 'Admin') {
        return await axiosAuth.post(`/api/Auth/AddRole`, JSON.stringify(vals), { headers: { 'Content-Type': 'application/json' } }).then((res) => true).catch((err) => false)
      } else {
        return await axiosAuth.delete(`/api/Auth/RemoveRole/${vals.userId}`, { headers: { 'Content-Type': 'application/json' } }).then((res) => true).catch((err) => false)
      }
    },
  })
}
