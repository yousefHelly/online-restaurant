...
function AllUsers({ initialData, size }: Props) {
  ...
  return (
    <PaginationProvider
      totalPages={users.data?.numOfPages || 1}
      showPagination={users.data && users?.data?.users.length > 0}
    >
      <div className='flex flex-col gap-5'>
        <div className='w-full self-center flex items-center justify-center'>
          <input
            type='search'
            placeholder='إبحث عن الأعضاء...'
            className='border dark:border-stone-600 dark:text-stone-300 px-3 w-1/3 py-2 rounded-r-md focus-within:border-dotted focus-within:border-main dark:focus-within:border-main focus-within:outline-none'
          />
        ...
        >
          ...
          <Table.Tbody>
            {users?.data?.users?.map((u) => (
              <UserItem key={u.userId} userData={u} />
            ))}
          </Table.Tbody>
        </Table>
      </div>
