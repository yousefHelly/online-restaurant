...
function TopDishes({filter, cardView='grid', admin=false, initialData, showSlider=false}:{filter: Sort, cardView?:'grid' | 'row', admin?: boolean, initialData?: Dishes, showSlider?: boolean}) {
  useAxiosAuth()
  ...
  return (
    <>
     {showSlider&&<Slider className={'w-full'} showArrows={data&&data?.meals.length>0 || false}>
      {
        data&&data.meals.length>0?data.meals.map((dish, i)=>{
          return(
