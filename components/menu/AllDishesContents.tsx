...
function AllDishesContents({
  ...
  const [filterList, setFilterList] = useState<{
    ...
    search: string;
  }>({ categories: [], chefs: [], price: { from: "", to: "" }, search: "" });
  const queryClient = useQueryClient();
  useEffect(() => {
    if (searchParams.get("f") != null && searchParams.get("n") != null) {
      searchParams.get("f") === "category"
        ? setFilterList({
            ...filterList,
            categories: [searchParams.get("n") as string],
          })
        : searchParams.get("f") === "chef"
        ? setFilterList({
            ...filterList,
            chefs: [searchParams.get("n") as string],
          })
  ...
  const dishes = useDishes(
    filterList.categories.join(",") || undefined,
    filterList.search || undefined,
    filterList.chefs.join(",") || undefined,
    +filterList.price.from || undefined,
    +filterList.price.to || undefined,
    1,
    selectedView.value,
    selectedSort.value,
    initialDishes
  );
  const categories = useCategories(initialCategories, 1, 100);
  const chefs = useChefs(initialChefs, 1, 100);
