...
import { useOrder } from "@/lib/api/useOrders";
...
function TrackingOrder({ id, admin }: Props) {
  ...
  return (
    <div className={admin ? "grid grid-cols-4 grid-rows-2 " : ""}>
      ...
      <div
        className={`${
          admin
            ? "col-span-3 my-8 mr-10 order-2 "
            : "w-full col-span-full my-8 lg:my-0"
        } grid lg:grid-cols-3 gap-5 justify-center items-center`}
      >
        <div className='lg:col-span-2 order-2 lg:order-first'>
          <h4 className='text-header dark:text-stone-300 font-bold text-2xl'>
            ملخص الطلبية
          </h4>
          <div className='py-2'>
...
TrackingOrder.Header = function Header({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <strong className='text-lg min-w-max font-bold text-main flex items-center gap-1'>
      {children} :{" "}
    </strong>
  );
};
