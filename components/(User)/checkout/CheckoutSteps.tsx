...
function CheckoutSteps({}: Props) {
  const [payment, setPayment] = useState<{ name: string } | undefined>(
    undefined
  );
  const [address, setAddress] = useState<Address>();
  const [staticAddition, setStaticAddition] = useState<
    PostOrder["staticAdditionOrders"]
  >([]);
  ...
  return (
    <>
          ...
          <CheckoutAccordionItem name='loginToAccount'>
            ...
            <Accordion.Panel>
              <CheckoutAccordionItem.PanelSkeleton
                action={"قم بتسجيل الدخول لتتمكن من المتابعة :"}
              >
                <button
                  className='self-center text-slate-50 dark:text-stone-900 dark:hover:text-main font-bold bg-main px-3 py-2 rounded-2xl  transition duration-150 hover:bg-transparent hover:text-main'
                  onClick={() => signIn()}
                >
                  تسجيل الدخول
                </button>
            ...
            <Accordion.Panel>
              ...
              >
                ...
                <button
                  ...
                  className='self-center flex gap-1 items-center px-3 py-2 rounded-2xl text-slate-50 dark:text-stone-900 dark:hover:text-main bg-main hover:bg-transparent hover:text-main transition duation-150'
                >
                  <Plus />
                  عنوان جديد
                </button>
                      ...
                      >
                        {({ checked }) => (
                          <div className='flex flex-col gap-3'>
                            <span
                              className={`px-3 py-2 cursor-pointer rounded-2xl transition duration-150 ${
                                checked
      ...
      <ViewActionTable
        disable={!session?.user || !payment || !address}
        action={setConfirmOrder}
        order={order}
        setOrder={setOrder}
            ...
            await postOrder.mutateAsync(
              { order: order },
              {
                onSuccess(data, variables, context) {
                  ...
                  emptyCart.mutate();
                  router.replace(
                    `/checkout/payment-confirmation?s=success&id=${data.id}`
                  );
                },
              }
            );
