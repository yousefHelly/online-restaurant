...
function DashboardAside({}: Props) {
  ...
  return (
    <motion.aside variants={containerVariants} animate={expanded ? 'expanded' : 'collapsed'} transition={{type:'spring'}} className={`admin-aside w-1/6 row-span-full min-h-screen flex flex-col justify-start items-start bg-stone-800 py-5`}>
            ...
            adminPages.map((page, i)=>{
                ...
                return(
                    page.relativePages.length>0?
                                ...
                                </NavLink>:    <NavLink component={Link} key={page.label+'-'+i} active={!!page.relativePages.find((rp)=>pathName==='/'+rp.link)} href={'/'+page.relativePages[0].link} leftSection={page.icon} color='#ffa006' variant={expanded?'light':'filled'} classNames={{root:` first-of-type:mt-5 my-3 ${!!page.relativePages.find((rp)=>pathName==='/'+rp.link)?'':'hover:!bg-transparent'} rounded-2xl px-4 text-right`, label:'', body:'', section:`!text-slate-50 !p-1 !m-0 w-8 h-8 flex items-center w-full justify-center transition duration-150 rounded-full ${!!page.relativePages.find((rp)=>pathName==='/'+rp.link)?'!text-stone-800':'hover:bg-main hover:!text-stone-800'}`}}/>
                    :
