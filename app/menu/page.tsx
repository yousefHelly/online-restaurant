...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: "قائمة الطعام",
    description: `استمتع بالأطباق من كل التصنيفات ! لدينا العديد من الصتنيفات في جو فاست فوود منها ${catnames.join(
      "، "
    )} ، كما ان لدينا العديد من الشيفات المتميزين مثل ${chefsnames.join(
      "، "
    )} . هؤلاء الشيفات المميزون يقدمون العديد من الأطباق اللذيذة مثل ${dishesnames.join(
      "، "
    )}`,
...
function FilterationTypeHeader({
  filter,
  link,
  children,
}: {
  ...
  return (
    <div className='flex flex-col gap-10 w-full my-5'>
      <div className='flex justify-between items-center dark:text-stone-300'>
        ...
        <Link
          className='flex justify-between items-center gap-3 transition duration-150 hover:text-main'
          href={`/menu/${link}`}
        >
          كل {filter}ات
          <ArrowLeft size={21} />
  ...
  return (
    <main className='flex min-h-max flex-col items-start pb-5 md:pb-10 px-8 md:px-24 overflow-x-hidden'>
      <FilterationTypeHeader filter='التصنيف' link='categories'>
        {categroiesData ? (
          <Categories showSlider={true} initialData={categroiesData} />
        ) : (
